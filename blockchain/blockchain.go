...
type BlockChain struct { //定义区块链
	LastHash []byte //当前区块链最后一个区块的哈希值，它并不是必须的，但可以避免我们在后面的函数编写中每次都去数据库中查找LastHash
	Database *badger.DB
}

type BlockChainIterator struct { //基于区块的迭代器来实现区块链的遍历
	CurrentHash []byte
	Database    *badger.DB
}
...
func InitBlockChain(address []byte) *BlockChain {
	...
	opts.Logger = nil //数据库的操作信息不输出到标准输出中
	...
	err = db.Update(func(txn *badger.Txn) error {
		...
		err = txn.Set(genesis.Hash, genesis.Serialize()) //哈希值作为Key值，区块序列化后的数据作为Value进行存储
		utils.Handle(err)
